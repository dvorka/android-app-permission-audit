...
public class DatabaseOpenHelper extends SQLiteOpenHelper {
    ...
    /*
	 * DatabaseOpenHelper :
	 * Création ou ouverture d'une base de données pour l'application
	 */
    ...
    /*
	 * createMinimalDb :
	 * Créations des tables minimales à la base de données
	 */
    ...
    /*
	 * fillCategory :
	 * Ajout d'une catégorie et lien avec ses permissions
    ...
    private void fillCategory(SQLiteDatabase db, String[] list, String categoryName) {
    	...
    	// Ajout de la categorie à la base de données
    	ContentValues values = new ContentValues();
    	values.put("name", categoryName);
    	long categoryId = db.insert("category", null, values);
    	
    	// Parcourt la liste des permissions de la catégorie
    	...
    	for (int i = 0; i < count; ++i) {
    		...
    		if (cursor.getCount() == 1) {
    			...
    			// Fait le lien entre la catégorie et la permission
    ...
    private void fillDb(SQLiteDatabase db) {
    	...
    	for (int i = 0; i < count; ++i) {
    		// ajoute chacune d'elles à la base de données
    		values = new ContentValues();
            values.put("name", permissionList[i]);
            db.insert("permission", null, values);
    	}
    	
    	// Ajout des catégories à la base de données
    	// LOCATION
    ...
    /*
	 * onCreate
	 * Appelé en cas de création d'une base de données 
	 */
    @Override
    ...
    /*
	 * onUpgrade
	 * Appelé en cas de changement de version de la base de données
	 */
	@Override
